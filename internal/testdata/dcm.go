...
// DCMFrameMeta is exported metadata for a single encapsulated frame.
...
// DCMPath returns a path under testdata/dcm/{uid}/.
func DCMPath(uid, name string) string {
	return filepath.Join(Root(), "dcm", uid, name)
}

// RequireDCMFrame loads an extracted DICOM frame and sidecar JSON metadata.
